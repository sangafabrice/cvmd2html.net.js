...
& {
  ...
  jsc.exe /nologo /target:$($DebugPreference -eq 'Continue' ? 'exe':'winexe') /win32res:$ResFile /resource:$TargetEmbeddedResFile /out:$(($ConvertExe = "$BinDir\cvmd2html.exe")) $(($ResourceSourcePath = "$BinDir\Resource.js")) "$(($SrcDir = "$PSScriptRoot\src"))\AssemblyInfo.js" "$PSScriptRoot\index.js" "$SrcDir\Program.js" "$SrcDir\Converter.js" "$SrcDir\MessageBox.js" "$SrcDir\Package.js" "$SrcDir\Param.js" "$SrcDir\Setup.js"
  $Env:Path = $EnvPath
  ...
  Remove-Item "$BinDir\*.res*",$ResourceSourcePath -Recurse -ErrorAction SilentlyContinue
}