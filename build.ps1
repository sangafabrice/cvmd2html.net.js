...
& {
  ...
  jsc.exe /nologo /target:$(if ($DebugPreference -eq 'Continue') { 'exe' } else { 'winexe' }) /win32res:$ResFile /r:$CoreDll /r:$AutomationDll /out:$(($ConvertExe = "$BinDir\cvmd2html.exe")) "$SrcDir\AssemblyInfo.js" "$PSScriptRoot\index.js" "$SrcDir\Program.js" "$SrcDir\Package.js" "$SrcDir\Param.js" "$SrcDir\Setup.js"
  $Env:Path = $EnvPath
